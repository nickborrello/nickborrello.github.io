    ...
    <div className="relative w-full h-full mb-4 group select-none bg-[#1a1814]">
      ...
      {hasMultiple && (
        <div className="absolute top-4 left-4 z-20 flex flex-col items-center gap-1">
             ...
             <button 
                onClick={handlePrev}
                className="text-[#f5f5f4] hover:text-[#eab308] drop-shadow-md transition-colors bg-black/60 hover:bg-black/80 rounded-sm p-0.5 backdrop-blur-sm border border-white/10"
             >
                <ChevronUp size={14}/>
             </button>

             {/* Indicators */}
             <div className="flex flex-col gap-2 items-center bg-black/50 py-2 px-1 rounded-sm backdrop-blur-sm border border-white/10 shadow-lg">
                {images.map((_, idx) => (
                  <div 
                    key={idx}
